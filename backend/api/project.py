...
def get_projects():
    ...
    projects_list = project_service.get_all(user_role=user_role, user_uuid=user_uuid, view_all=show_all)
...
def get_project_detail(uuid):
    ...
    if not success:
        code = 403 if message == "Permission denied" else 404
        return (
...
def get_project_files(uuid):
    ...
    success, message, files = project_service.get_file_tree(uuid, path, user_role, user_uuid)

    if not success:
        code = 403 if message == "Permission denied" else (404 if "not found" in message else 500)
        return (
            jsonify({"level": "error", "message": message}),
            code,
        )
...
def get_file_content(uuid):
    ...
    if not path:
        return (
            jsonify({"level": "error", "message": "Path is required"}),
            400,
        )

    success, message, content = project_service.get_file_content(uuid, path, user_role, user_uuid)
...
def create_project():
    ...
    if success:
        ...
        code = 403 if "Permission" in message else 500
        return (
            jsonify(
                {
                    "level": "error",
...
def update_project(uuid):
    ...
    if success:
        ...
        code = 403 if "Permission" in message else (404 if "not found" in message else 500)
