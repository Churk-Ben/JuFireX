...
class AuditService:
    ...
    @staticmethod
    def log_user_action(
        user_id: int, action: str, resource: str = "", details: str = ""
    ):
        """记录用户操作日志"""
        # TODO: 实现到数据库或日志文件
        import logging
        ...
        logger.info(
            f"User {user_id} performed {action} on {resource}. Details: {details}"
        )

    @staticmethod
    def log_security_event(
        event_type: str, user_id: Optional[int] = None, details: str = ""
    ):
        """记录安全事件"""
        import logging
