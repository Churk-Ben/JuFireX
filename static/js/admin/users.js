...
function handleUserAction(event) {
    ...
    } else if (button.classList.contains('view-user-details')) {
    ...
    API.get(`/api/users/${userId}`)
        .then(data => {
            if (data.success) {
                ...
                document.getElementById('userDetailsContent').innerHTML = createUserDetailsHtml(user);
                new bootstrap.Modal(document.getElementById('userDetailsModal')).show();
            } else {
                showNotification('获取用户详情失败：' + data.message, 'error');
            }
        })
        .catch(handleApiError('获取用户详情失败'));
}
    ...
    return `
        ...
        <div class="d-flex justify-content-end mt-3">
            <button class="btn btn-sm btn-outline-warning me-2" onclick="toggleUserStatus(${user.id}, ${user.is_active})">
                <i class="fas fa-${user.is_active ? 'ban' : 'check'} me-1"></i> ${user.is_active ? '禁用用户' : '启用用户'}
            </button>
